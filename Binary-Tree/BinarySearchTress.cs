    ...
    public class BinarySearchTree<Tdata> where Tdata : IComparable<Tdata>
    {
        ...
        public void Insert(Tdata _data)
        {
            ...
            while (currentNode != null)
            {
                ...
                {
                    Console.WriteLine("No Duplicate Data");
                    return;
                }
            }
        }
        ...
        {
            return FindNodeAndParent(_data).Node != null;
        }
        ...
        public void Delete(Tdata _data)
        {
            ...
            if (nodeAndParentInfo.Node is null) return;
            if (nodeAndParentInfo.Node.Left is not null && nodeAndParentInfo.Node.Right is not null)
            {
                Delete_Has_TwoChilds(nodeAndParentInfo.Node);
            }
            else if (nodeAndParentInfo.Node.Left is not null ^ nodeAndParentInfo.Node.Right is not null)
            {
                Delete_Has_OneChild(nodeAndParentInfo.Node);
            }
            else
            {
                Delete_Leaf_Node(nodeAndParentInfo);
            }
            //InternalDelete(_data);
        }
        private void Delete_Has_TwoChilds(TreeNode nodeToDelete)
        {
            TreeNode smallestNode = nodeToDelete.Right;
            TreeNode parentOfSmallest = null;
            while (smallestNode.Left is not null)
            {
                parentOfSmallest = smallestNode;
                smallestNode = smallestNode.Left;
            }
            if (parentOfSmallest is null)
            {
                nodeToDelete.Right = smallestNode.Right;
            }
            else
            {
                parentOfSmallest.Left = smallestNode.Right;
            }
            nodeToDelete.Data = smallestNode.Data;
        }
        private void Delete_Has_OneChild(TreeNode nodeToDelete)
        {
            TreeNode nodeToReplace = nodeToDelete.Left ?? nodeToDelete.Right;
            nodeToDelete.Right = nodeToReplace.Right;
            nodeToDelete.Left = nodeToReplace.Left;
            nodeToDelete.Data = nodeToReplace.Data;
            return;
        }
        private void Delete_Leaf_Node(NodeAndParent nodeToDelete)
        {
        ...
        private NodeAndParent FindNodeAndParent(Tdata _data)
        {
            ...
            while (currentNode != null)
            {
                ...
                {
                    nodeAndParent = new NodeAndParent() { Node = currentNode, Parent = parent, IsLeft = left };
                    break;
                }
                else if (currentNode.Data.CompareTo(_data) > 0)
                {
